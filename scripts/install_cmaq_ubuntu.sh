...
# install compilers
sudo apt install -y make gcc g++ cpp gfortran openmpi-bin libopenmpi-dev

# install jasper stuff
sudo add-apt-repository "deb http://security.ubuntu.com/ubuntu xenial-security main"
sudo apt update
sudo apt install -y libjasper1 libjasper-dev libpng-dev
...
sed -i 's+mpi_lib_gcc+/usr/lib/x86_64-linux-gnu/openmpi+g' config_cmaq.csh
